...
class SslCommerzPaymentController extends Controller
{
    public function exampleEasyCheckout(Request $request)
    {
        if (!Auth::id()) {
            ...
            $productdetails = json_decode(json_encode(Cart::join('online__courses', 'carts.online_course_id', '=', 'online__courses.id')
            ->whereIn('carts.id', $data)->get()));
            $details = [];
            $items = [];
            $quantities = [];
            ...
            foreach ($items as $key => $item) {
                $item_detalis = $item;
                $item_detalis->qty = $quantities[$key];
                array_push($details, $item_detalis);
            }

            return view('exampleEasycheckout', compact('details'));
        }
    }
    ...
    public function payViaAjax(Request $request)
    {
        ...
        $update_product = DB::table('orders')
            ...
            ->updateOrInsert([
                ...
                'transaction_id' => $post_data['tran_id'],
                'currency' => $post_data['currency'],
                'product_detalis' => $data->cart_info,
            ]);
        Cart::where('user_id', Auth::id())->delete();
        $sslc = new SslCommerzNotification();
