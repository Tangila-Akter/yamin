  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Add Management Team</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
          <form action="{{ url('/uploadmanagement_team') }}" method="post" enctype="multipart/form-data">
              @csrf
  ...
  <table id="" class="display">
    ...
    <tbody>
      <tr style="background-color: #ffffff; color:black;" class="item">
        ...
        <td>{{ $data->YouTube }}</td>
        <td>{{ $data->Linkedin }}</td>
        <td><img height="100" width="100" src="management_team/{{ $data->image }}" ></td>
        <td><a class="btn btn-warning" href="{{ url('/deletemanagement_team',$data->id) }}">Delete</a>&nbsp
          <a class="btn btn-primary" href="{{ url('/updatemanagement_team',$data->id) }}">Edit</a></td> 
      </tr>
    </tbody>
    @endforeach
