  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Add Offline Course</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
          <form action="{{ url('/uploadOffline_Course') }}" method="post" enctype="multipart/form-data">
              @csrf
    ...
    <tbody>
      <tr style="background-color: #ffffff; color:black;" class="item">
        <td>{{ $data->title }}</td>
             ...
             <td><img height="100" width="100" src="Offline_Course/{{ $data->image }}" ></td>
             <td><a class="btn btn-warning" href="{{ url('/deleteOffline_Course',$data->id) }}">Delete</a>&nbsp<a class="btn btn-primary" href="{{ url('/updateOffline_Course',$data->id) }}">Edit</a></td> 
      </tr>
    </tbody>
    @endforeach
